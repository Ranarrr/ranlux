...
void utility::normalizeangle( angle ang ) {
	for( int axis = 0; axis < 3; ++axis ) {
		while( ang[ axis ] > 180.f ) {
			ang[ axis ] -= 360.f;
		}
...
bool utility::is_playervisible( c_csplayer *player, const Vector end ) {
	if( end.IsZero() )
		return false;

	CGameTrace	tr;
	Ray_t		ray;
	CTraceFilterSkipTwoEnts filter( globals::localplayer, player );

	ray.Init( globals::localplayer->get_eyepos(), end );

	globals::enginetrace->TraceRay( ray, MASK_REAL_VISIBLE, ( ITraceFilter* ) &filter, &tr );

	if( tr.fraction == 1.f || tr.m_pEnt == player )
		return true;
